...
class ProfilController extends AbstractController
{
    ...
    public function liste(ProduitRepository $produitRepository): Response
    {
        ...
        /**
     * @Route("/detail-commande-{id}", name="app_profil_detail_commande", requirements={"id"="\d+"})
     */
    public function detailCommand(Commande $commande)
    {
        if( $commande->getClient() == $this->getUser()){
            return $this->render("profil/detail_commande.html.twig", ["commande" => $commande]);
        }
        throw $this->createAccessDeniedException("Vous n'avez psas accès à cet URL");

        // $this->addFlash("danger", "ERREUR 403 : Vous n' avez pas accès à cet URL");
